const SCHEMA = {
  ...
  Customer_Risk_Intelligence: [
    ...
    'RTO Count', 'Risk Level', 'Fraud Loss',
    // New columns (appended, not reordered, so existing sheet data/columns
    // stay untouched - add these 3 headers anywhere in row 1; findCol()
    // matches by name so their physical position doesn't matter):
    'Delivered Count', 'Customer Tag', 'Pattern Notes',
  ],
...
// Flexible header lookup: matches ignoring case/spacing/punctuation, so
// "Claim Value" also matches a sheet header of "Claim Value (₹)", etc.
// Falls back to a prefix match if no exact normalized match is found.
