...
const UNDER_SETTLEMENT_TOLERANCE = 5; // Rs. - ignore tiny rounding differences
...
async function processReconciliationFile(buffer) {
  ...
  if (!rows.length) return { count: 0, discrepancies: 0, underSettlementCount: 0 };
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (invoiceAmount !== undefined) {
      ...
      if (shortfall > UNDER_SETTLEMENT_TOLERANCE) {
        notes.push(`⚠️ Under-Settlement: Rs.${shortfall} short (extra deductions)`);
        underSettlementCount += 1;
      }
    } else {
      notes.push('No matching order found');
      discrepancies += 1;
    }

    outRows.push([
      paymentDate, subOrderId, 'Settled', grossSale, marketplaceFee,
      '', netSettlement, 'Reconciled', notes.join(' | '),
    ]);
  }
